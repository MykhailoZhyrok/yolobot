...
Місто та номер відділення Нової Пошти: ${cityPost}`;
    ...
    if (userInfo[chatId]) {
        if (userInfo[chatId].status === 'awaitingName') {
            ...
            console.log(userInfo[chatId].status);
...
console.log(msg.photo)
 if(userInfo[chatId]){if(userInfo[chatId].status='awaitingPhoto') {bot.sendPhoto(groupChatId, photoId, { caption: `chatID${chatId}, Номер замовлення: ${orderNumbers}` }).then(() => {
    ...
    .catch((error) => {
      ...
      const errorMessage = 'Помилка при відправці фотографії адміну. Спробуйте ще раз пізніше.';
...
bot.on('callback_query', (query) => {
    ...
    if (data.startsWith('yolo')) {
        ...
        const cart = userCarts[chatId];
        console.log(`Quantity: ${userSelections[chatId].quantity}`)
...
Номер замовлення: ${orderNumber}`, agreeSell)}
        ...
        console.log(chatId)
        bot.deleteMessage(chatId, messageId).then(() => {
            bot.sendMessage(chatId, `${totalSum} Номер карти для оплати(ПІБ):
${cartNumber}
...
adbot.setMyCommands([
    { command: '/start', description: 'Початкове вітання' },
    { command: '/add_product', description: 'Додати товар' },
]);
...
adbot.on('message', async (msg, match) => {
    ...
    if (text === '/start') {
        adbot.sendMessage(adChatId, `Бот працює`, confirmKeyboard);
...
adbot.on('callback_query', (query) => {
    const chatId = adChatId;
    const messageId = query.message.message_id;
    const data = query.data;

    // Якщо data містить потрібну вам інформацію, наприклад, команду, то виконайте відповідні дії
    if (data === 'confirm_order') {
        adminState[chatId] = { status: 'inputID' };

        // Відправте запит користувачу
        adbot.deleteMessage(chatId, messageId).then(() => {
            bot.sendMessage(chatId, 'Введіть ID, який підтвердити:');
        }).catch((error) => {
            console.error('Помилка при видаленні повідомлення:', error);
        });
    }
